...
/*router.post('/add', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    client.query('INSERT INTO category_master(ctm_type, ctm_username, ctm_status) values($1,$2,0)',[req.body.ctm_type,req.body.ctm_username]);
    // SQL Query > Select Data
...
router.post('/add', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    var singleInsert = 'INSERT INTO category_master(ctm_type,ctm_username,ctm_status) values($1,$2,0) RETURNING *',
        params = [req.body.ctm_type,req.body.ctm_username]
...
/*router.post('/edit/:ctmId', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    // SQL Query > Insert Data
    client.query('UPDATE category_master SET ctm_type=$1, ctm_updated_at=now() where ctm_id=$2',[req.body.ctm_type,id]);
    // SQL Query > Select Data
    const query = client.query('SELECT * FROM category_master');
    query.on('row', (row) => {
      results.push(row);
...
router.post('/edit/:productId', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    var singleInsert = 'update category_master set ctm_type=$1, ctm_updated_at=now() where ctm_id=$2 RETURNING *',
        params = [req.body.ctm_type,id];
    client.query(singleInsert, params, function (error, result) {
        results.push(result.rows[0]); // Will contain your inserted rows
...
router.post('/category/limit', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    const strqry =  "SELECT ctm.ctm_id, ctm.ctm_type "+
                    ...
                    "and LOWER(ctm_type) LIKE LOWER($1) "+
                    "order by ctm.ctm_id desc LIMIT $2 OFFSET $3";
